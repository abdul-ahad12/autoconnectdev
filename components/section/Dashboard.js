...
const Dashboard = ({ bookings, role }) => {
  ...
  return (
    <div
      className={`w-full flex justify-center bg-white overflow-hidden relative ${
        role !== "CUSTOMER" && "lg:h-screen"
      } `}
    >
      <div
        className={`w-[90%] flex ${
          role !== "CUSTOMER" && "pt-[8rem]"
        } gap-[2rem]`}
      >
        {/* left bar */}
        <div className="base:hidden w-[25%] lg:flex items-center flex-col   justi rounded-lg border border-graycolor2 p-7  ">
